...
public class MotionDetector {
            ...
            while (isRunning.get()) {
                ...
                if (now-lastCheck > checkInterval) {
                    ...
                    if (nextData.get() != null) {
                        ...
                        // rContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("MotionDetected", "Image Data");
                        ...
                        } else if (detector.detect(img, nextWidth.get(), nextHeight.get(), senstivity)) {
                            // check
                        // rContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("MotionDetected", "Movement");
    ...
    private int minLuma = 100;
    private MotionDetectorThread worker;

    private int senstivity=20;
    ...
    public MotionDetector(SurfaceView surface, int sensi) {
        senstivity=sensi;
    ...
    private Camera getCameraInstance(){
        ...
        try {
            if (Camera.getNumberOfCameras() >= 2) {
                ...
                // rContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("MotionDetected", "Camera started");
            } else {
                c = Camera.open();
            }
        }
